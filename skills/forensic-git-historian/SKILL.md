...
1. **Workspace Orientation**: Execute the `Shell` tool with `git status` to identify modified and untracked files. (Note: `git status` is required; execute it sequentially, never chained).
2. **Mandatory Watchtower Audit**: Execute `python <this-skill-folder>/scripts/security_audit.py` to programmatically verify `.gitignore` integrity and cross-reference against your `git status` findings. **Do not proceed if this exits with an error code.**
3. **Semantic Ingestion**: Execute `sem diff --format json` (never `git diff`) to extract exact structural and logical changes (e.g., added parameters, changed return types, new classes).
4. **Historical Alignment**: Execute `sem diff --from HEAD~3 --to HEAD --format json` (never `git log`) to establish context on the repository's recent architectural trajectory.
5. **Validation Loop**: Verify the `entityType` in JSON outputs. If it reads `chunk`, semantic extraction failed for that file. Explicitly check for `renamed` or `moved` tags before assuming an entity was deleted.
